...
# weight and biases of the convolutional layer #2
...
# weight and biases of the convolutional layer #3
...
# weight and biases of the convolutional layer #4
...
# weight and biases of the fully-connected relu layer #5
...
# weight and biases of the fully-connected softmax layer #6
...
# flatten the output from convolutional later #4
Y4flat = tf.reshape(Y4, shape=[-1, 7 * 7 * M])
# fully-connected relu layer #5
Y5 = tf.nn.relu(tf.matmul(Y4flat, W5) + b5)
#fully-connected softmax layer #6
...
train_writer = tf.summary.FileWriter('summary/conv-train', sess.graph)
test_writer = tf.summary.FileWriter('summary/conv-test')
...
for step in range(20001):
    ...
    if step % 50 == 0:
        summary, train_accuracy, train_cross_entropy = sess.run([merged, accuracy, cross_entropy], feed_dict={X: batch_X, Y_: batch_Y})
        train_writer.add_summary(summary, step)

        summary, test_accuracy, test_cross_entropy = sess.run([merged, accuracy, cross_entropy], feed_dict={X: mnist.test.images, Y_: mnist.test.labels})
